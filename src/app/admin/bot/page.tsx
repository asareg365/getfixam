...
export default function WhatsAppBotPage() {
  ...
  async function handleSaveConfig(e: React.FormEvent) {
    ...
    setDoc(configRef, updateData, { merge: true })
      .then(() => {
        toast({ title: 'Configuration Saved', description: 'Meta API settings updated.' });
      })
      .catch(async (err) => {
        const permissionError = new FirestorePermissionError({
          path: configRef.path,
    ...
    <div className="space-y-10">
      ...
      <Alert className="rounded-[24px] border-primary/20 bg-primary/5">
        ...
        <AlertDescription className="font-medium">
          Messages sent via this dashboard are currently <b>simulations</b>. Real device connections require Meta Business credentials.
        </AlertDescription>
      </Alert>
      ...
      <Tabs defaultValue="simulator" className="space-y-8">
          ...
          <Card className="border-none shadow-xl rounded-[40px] overflow-hidden">
            ...
            <CardContent className="p-10 pt-0 space-y-8">
              <div className="grid md:grid-cols-2 gap-12">
                ...
                <div className="space-y-6">
                    <label className="text-sm font-bold uppercase tracking-wider text-muted-foreground/80">Visual Preview</label>
                    {lastResult ? (
                        ...
                        <div className="bg-muted/30 rounded-[32px] p-12 border border-dashed border-muted-foreground/30 flex flex-col items-center justify-center text-center h-full min-h-[250px]">
        ...
        <TabsContent value="config">
          <Card className="border-none shadow-xl rounded-[40px] overflow-hidden max-w-3xl mx-auto">
